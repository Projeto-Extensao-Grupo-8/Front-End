...
const PerfilPsicologo = () => {
  ...
  // ── Dias do mês que possuem agendamentos (para bolinhas no calendário) ──
  const [diasComAgendamento, setDiasComAgendamento] = useState([]);
 
  const usuario = JSON.parse(localStorage.getItem("usuario"));
  const idFuncionario = usuario?.idFuncionario;
 
  // ── Transforma appointmentsByPsychologistId no formato do grid ──
  // Ajuste os campos (appt.horario, appt.duracao, appt.status) conforme retorno real da sua API
const horarios = Array.isArray(appointmentsByPsychologistId)
  ...
  // ── Busca appointments sempre que a data selecionada mudar ──
  useEffect(() => {
    getAppointmentsByPsychologistId(idFuncionario, dataSelecionada);
  }, [dataSelecionada]);

  console.log(appointmentsByPsychologistId);
 
  // ── Extrai os dias do mês que têm agendamento para pintar as bolinhas ──
 useEffect(() => {
          ...
          <div className={styles.mainGrid}>
            ...
            <div className={styles.rightCol}>
              ...
              <div className={styles.card}>
                ...
                {/* Passa os dias com agendamento para o calendário pintar as bolinhas */}
                <MiniCalendar
